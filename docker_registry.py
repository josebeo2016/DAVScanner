...
from utils import mkpdirs, request, request_and_check, getListOfFiles
...
class LocalRegistry(object):
    tmp_folder = tempfile.mkdtemp(prefix='dockerfile-')
    ...
    def get_file_list(self, image, layer, isFull=False):
        # print("get_file_list")
        ...
        os.remove(image_tar)
        layer_dir = join(repo_dir,"blobs",layer)
        layer_ext = join(layer_dir,"tmp")
        with tarfile.open(join(layer_dir,'layer.tar')) as tar:
            mkpdirs(layer_ext)
            # print("Get_file_list: {}".format(layer_ext))
            # os.chmod(join(layer_dir,"tmp"),777)
            tar.extractall(join(layer_dir,"tmp"))
        if isFull:
            return getListOfFiles(layer_ext,"")
        else:
            return getListOfFiles(layer_ext,layer_ext)
        
    def count_file(self, image, layer):
        repo_dir = join(self.tmp_folder, image.repository)
        count = 0
        size=0
        blobs_dir = join(repo_dir, 'blobs')
    ...
    def get_history(self,image):
        logger.debug('Get history for {}'.format(image))
        image = self._client.images.get(image.name)
